...
ggplot(df.s, aes(x=csum.TRA / len.TRA, y=csum.TRB / len.TRB)) + 
  ...
  scale_x_continuous(name = "TCR alpha contacts, normalized", limits=c(0,2)) + 
  scale_y_continuous(name = "TCR beta contacts, normalized", limits=c(0,2)) +
...
Check for polar property seprately (with charge there is no way do any statistics):

```{r}
df.cc3 <- merge(df.cc, subset(aa_prop, property == "polar"), all=T)
df.cc3$value <- as.factor(df.cc3$value)
t.test(contacts ~ value, data = df.cc3)
```
...
heatmap.2(contact_mat_norm, col=colorpanel(100, "#f0f0f0", "#fdbb84", "#e34a33"),
          hclustfun = function(d) hclust(d, method="ward"),
          #ColSideColors = colgen(colnames(contact_mat), "polar", "white", "white", "black"), 
          RowSideColors = colgen(rownames(contact_mat), "core", "white", "grey", "black"),
          density.info = "none", trace="none")
...
## Estimating the number of CDR3 contacts

We will first estimate ``P(contacting|aa_tcr,pos_norm)``, conditional probability of CDR3:antigen contact for a given CDR3 amino acid. Note that

- ``pos_norm`` is a position of amino acid relative to CDR3 center
- When ``P`` approaches ``1`` (i.e. always forms a contact in this position) we'll replace it by ``(k+1)/(k+2)``, where ``k`` is the total number of times given amino acid is observed in a given position. Otherwise, we'll have troubles estimating mean number of contacts from poisson distribution for this case.

```{r}
df.0 <- subset(df, tcr_region == "CDR3")

compute_pos_norm <- function(pos_tcr, len_tcr) {
  round(pos_tcr - len_tcr[1] / 2)
  # round((pos_tcr - len_tcr[1] / 2)/3)*3
}

# P(contact|aa,pos)
p.cap <- ddply(df.0, .(pdb_id, tcr_chain, pos_tcr, aa_tcr), summarize,
               pos_norm = mean(compute_pos_norm(pos_tcr, len_tcr)),
               has.contact = max(ifelse(distance <= DIST_THRESHOLD, 1, 0)))
p.cap <- ddply(p.cap, .(aa_tcr, pos_norm), summarize, 
               prob = min(mean(has.contact),
                          # Use (k+1)/(k+2) when P=1, otherwise we'll get problems with Poisson distr
                          (length(has.contact) + 1)/(length(has.contact) + 2)))

if(FALSE){
# P(aa|pos)
p.ap <- ddply(df.0, .(pdb_id, tcr_chain, pos_tcr, aa_tcr), summarize,
               pos_norm = mean(compute_pos_norm(pos_tcr, len_tcr)),
               count = 1)
p.ap <- ddply(p.ap, .(aa_tcr, pos_norm), summarize,
               total = sum(count))
p.ap <- ddply(p.ap, .(pos_norm), transform,
               prob = total / sum(total))

# P(pos)
p.p <- ddply(df.0, .(pdb_id, tcr_chain, pos_tcr), summarize,
               pos_norm = mean(compute_pos_norm(pos_tcr, len_tcr)),
               count = 1)
p.p <- ddply(p.p, .(pos_norm), summarize,
               total = sum(count))
p.p$prob <- p.p$total / sum(p.p$total)
}
```

Plot the heatmap for ``P(contacting|aa_tcr,pos_norm)``.

```{r}
ggplot(p.cap, aes(x=aa_tcr, y=pos_norm, fill=prob)) + 
  geom_tile() + 
  scale_fill_gradient2("Contact probability", low="#f7fcb9", mid="#addd8e", high="#31a354", limits=c(0,1)) +
  xlab("CDR3 amino acid") + ylab("Position relative to CDR3 center") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
```

Now compute number of contacting CDR3 amino acids for our dataset using ``P(contacting|aa_tcr,pos_norm)``. For the total number of contacts, we'll approximate the contact number using poisson distribution as follows ``Poiss(0)=exp(-lambda)=1-P(contacting|aa_tcr,pos_norm)`` therefore ``E[contacts]=-log(1-P(contacting|aa_tcr,pos_norm))``.

```{r}
compute_contact_prob <- function(aa_tcr, pos_tcr, len_tcr) {
  pos_norm <- compute_pos_norm(pos_tcr, len_tcr)
  mapply(function(x, y) p.cap$prob[which(p.cap$pos_norm == x & p.cap$aa_tcr == y)], pos_norm, aa_tcr)
}

df.o <- ddply(df.0, .(pdb_id, tcr_chain, pos_tcr), summarize,
              contacts.obs = max(ifelse(distance <= DIST_THRESHOLD, 1, 0)),
              contacts.obs.wt = sum(ifelse(distance <= DIST_THRESHOLD, 1, 0)))
df.o <- ddply(df.o, .(pdb_id, tcr_chain), summarize,
              contacts.obs = sum(contacts.obs),
              contacts.obs.wt = sum(contacts.obs.wt))
df.e <- ddply(subset(df.0, pos_antigen==0), .(pdb_id, tcr_chain), summarize,              
              contacts.exp = sum(compute_contact_prob(aa_tcr, pos_tcr, len_tcr)),
              # Using poisson distribution to estimate number of contacats per aa:
              contacts.exp.wt = sum(-log(1-compute_contact_prob(aa_tcr, pos_tcr, len_tcr)))) 

df.oe <- merge(df.o, df.e)

df.oe <- subset(df.oe, contacts.obs > 0)
```

Fitting number of contacting amino acids in CDR3:

```{r}
ggplot(df.oe, aes(x=contacts.obs, y=contacts.exp)) + 
  geom_smooth(method="lm") + 
  geom_point(shape=1, size=3) + 
  scale_x_continuous("Observed contacting amino acids", limits = c(0,10), 
                     breaks=seq(0,10,2), expand=c(0,0)) +
  scale_y_continuous("Expected contacting amino acids", limits = c(0,10), 
                     breaks=seq(0,10,2), expand=c(0,0)) +
  theme_bw()
  
summary(lm(formula = contacts.obs ~ contacts.exp, data=df.oe))
```

Fitting total number of contacts for CDR3 (i.e. weighted number of contacting amino acids):

```{r}
ggplot(df.oe, aes(x=contacts.obs.wt, y=contacts.exp.wt)) + 
  geom_smooth(method="lm") + 
  geom_point(shape=1, size=3) + 
  scale_x_continuous("Observed CDR3:antigen contacts", limits = c(0,29),
                     breaks=seq(0,28,4), expand=c(0,0)) +
  scale_y_continuous("Expected CDR3:antigen contacts", limits = c(0,29),
                     breaks=seq(0,28,4), expand=c(0,0)) +
  theme_bw()

summary(lm(formula = contacts.obs.wt ~ contacts.exp.wt, data=df.oe))
```

...
plot_ag <- function(ag_seq) {
  ggplot(subset(df.m, antigen_seq == ag_seq), 
         aes(x=pos_antigen, y=pos_tcr, 
             fill=distance, color=distance <= 6)) + 
    geom_tile(size=1) + 
    scale_color_manual("Less than threhsold", values =  c("grey50", "black")) +
    facet_wrap(~tcr_region_seq, scales="free") +
    scale_fill_gradient2("Distance, A", limits = c(0, 20), midpoint = 10, 

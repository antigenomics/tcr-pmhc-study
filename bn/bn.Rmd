...
df <- read.table("structure.txt", header = T, sep="\t") %>%
  ...
  select(pdb_id, tcr_region, tcr_chain, pos_tcr, len_tcr, aa_tcr, pos_antigen, len_antigen, aa_antigen, mhc_type, distance) %>%
...
df <- df %>% select(tcr_region, tcr_chain, pos_rel_tcr, aa_tcr, pos_rel_antigen, aa_antigen, mhc_type, contact)

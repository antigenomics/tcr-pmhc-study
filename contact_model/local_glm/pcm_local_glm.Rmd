...
ggplot(df.contact.sum, aes(x=tcr_region, group = tcr_region, y = contacts, fill = tcr_region)) +
  ...
  xlab("") + ylab("Antigen contacts (closest atom distance < 4.5Å)") +
...
impute_knn_mean <- function (X_train, y_train, X_val, y_val, X_test, .k) {
  # For each object in the train data find its 5 closest neighbours and take take the mean of their coefficients as predictions.
  ...
  # For each object in the validation data find its 5 closest neighbours in the training data and take the mean of their coefficients as predictions.
  ...
  # Merge training and validation data and predict the final missing values.
...
impute_knn_dist <- function (X_train, y_train, X_val, y_val, X_test, .k) {
  # For each object in the train data find its 5 closest neighbours and take take the weighted mean of their coefficients as predictions.
  res_tr = sapply(1:nrow(X_train), function (row_i) { 
    tmp = sapply(1:nrow(X_train), function (row_j) {
      if (row_i != row_j) { sqrt(sum((X_train[row_i, ] - X_train[row_j, ]) ^ 2)) } 
      else { 20 }
  ...
  # For each object in the validation data find its 5 closest neighbours in the training data and take the weighted mean of their coefficients as predictions.
  res_val = sapply(1:nrow(X_val), function (row_i) { 
    tmp = sapply(1:nrow(X_train), function (row_j) {
      sqrt(sum((X_val[row_i, ] - X_train[row_j, ]) ^ 2))
    })
...
impute_reg <- function (X_train, y_train, X_val, y_val, X_test) {
  # Build the regression model on the train data in order to compute error on the training data
  ...
  # Compute error on the validation data
  X_df = data.frame(X_val)
  res_val = sapply(1:nrow(X_val), function (row_i) { 
    predict(model, X_df[row_i, ])
  })
  
  # Build the model on the merged train / validation datasets in order to predict the missing values
  merged = rbind(X_train, X_val)
  merged_y = c(y_train, y_val)
...
impute_reg_int <- function (X_train, y_train, X_val, y_val, X_test) {
  # Build the regression model on the train data in order to compute error on the training data
  model = lm(aa_data$y ~ .^2, data = data.frame(aa_data$X))
...
imp_res[["reg_int"]] = eval_model(aa_data, impute_reg_int)
...
imputed = impute_reg_int(aa_data$X[aa_data$train[[1]], ],
...
```{r}
save(df.ca.mean, df.aa.coef, contact_glm, df.energies, file="../evaluation/pcm_local_glm.RData")

...
def train_model(max_pos, n_clust, coord, layers, 
                ...
                hist, model_list, 
                how = "no", scale = "no", 
                coord_fun = "onehot", model_fun = "dense"):
    ...
    if model_name not in hist:
        ...
        elif model_fun == "dense_pos":
            model_fun = dense_pos_model
            # add positions
            X_can = [X_can, np.array([((x % 12) + 1) / 12. for x in range(X_can.shape[0])])]
            X_cdr = [X_cdr, np.array([((x % 12) + 1) / 12. for x in range(X_cdr.shape[0])])]
        else:
            print("Unknown parameter", coord_fun)
            return 0

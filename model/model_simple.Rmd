...
```{r}
df.pred.trimmed = df.pred[df.coef.1, on = "aa_pair"][distance_CA <= 15]
```
...
compute_contact_map = function(mhc_type, tcr_chain, tcr_region, cdr_seq, ag_seq, id = "tmp") {
  ...
  merge(df.pairs %>% select(id, mhc_type, tcr_chain, tcr_region, pos_tcr_c, pos_antigen_c, aa_antigen, aa_tcr, aa_pair), 
        df.pred.trimmed %>% select(mhc_type, tcr_chain, tcr_region, pos_tcr_c, pos_antigen_c, aa_pair, p))
}
```
...
print(cm.tmp %>% group_by(id) %>% summarize(contacts = sum(ifelse(p < 0.1, 0, p), na.rm = T)))
